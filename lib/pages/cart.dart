          ...
          children: [
            for (final item in myCart)
              ListTile(
                title: Row(
                  children: [
                    IconButton(
                      onPressed: () => cartNotifier.decrementProductByOne(item.product),
                      icon: const Icon(Icons.remove_circle)
                    ),
                    Text('€${(item.product.price).toStringAsFixed(2)} (x${item.quantity})'),
                    IconButton(
                      onPressed: () => cartNotifier.incrementProductByOne(item.product), 
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  Text('Totale carrello: €${totalCost.toStringAsFixed(2)}',
                  style: TextStyle(
